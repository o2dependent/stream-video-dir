<script lang="ts">
	...
	export let downloadingIds: string[];

	$: isDownloading = downloadingIds?.includes?.(id) ?? false;

	const startDownload = () => {
		fetch(`/video/download/episode/${id}`, {
			method: "POST",
		});
		isDownloading = true;
	};
...
<button
	...
	class="p-4 bg-neutral-700/0 hover:bg-neutral-700/5 opacity-50 hover:opacity-100 shadow-sm hover:shadow-md transition-all duration-75 md:flex"
>
	<h3 class="text-lg text-left font-semibold w-full">{name}</h3>
	{#if isDownloading}
		<svg
