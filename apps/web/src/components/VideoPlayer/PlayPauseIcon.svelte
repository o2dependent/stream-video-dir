...
<svg
	class="play-pause-icon transition-all fill-none {className}"
...
>
	...
	{:else}
		...
		/>
		<path
			class="origin-center pause-path"
			d="M5.669 4.76a1.469 1.469 0 0 1 2.04-1.177c1.062.454 3.442 1.533 6.462 3.276c3.021 1.744 5.146 3.267 6.069 3.958c.788.591.79 1.763.001 2.356c-.914.687-3.013 2.19-6.07 3.956c-3.06 1.766-5.412 2.832-6.464 3.28c-.906.387-1.92-.2-2.038-1.177c-.138-1.142-.396-3.735-.396-7.237c0-3.5.257-6.092.396-7.235Z"
		/>
		<path
			class="origin-center pause-path"
...
<style lang="postcss">
	:global(svg.play-pause-icon path.play-path) {
		display: block;
	}

	:global(svg.play-pause-icon path.pause-path) {
		display: none;
	}
	:global(.paused svg.play-pause-icon path.play-path) {
		display: none;
	}

	:global(.paused svg.play-pause-icon path.pause-path) {
		display: block;
	}
</style>
